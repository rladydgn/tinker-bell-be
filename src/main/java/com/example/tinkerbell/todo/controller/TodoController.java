...
public class TodoController {
	...
	@GetMapping(value = "/{id}")
	public ResponseEntity<TodoDto.Response> getTodo(@PathVariable int id, @Login User user) {
		return ResponseEntity.ok(todoService.getTodo(id, user));
	}

	@Operation(summary = "유저의 todo 목록 조회")
	@GetMapping
	public ResponseEntity<List<TodoDto.Response>> getTodoList(@Login User user) {
		return ResponseEntity.ok(todoService.getTodoList(user));
	}

	@Operation(summary = "todo 생성")
	@PostMapping
	public ResponseEntity<Void> saveTodo(@RequestBody TodoDto.Request todoDto, @Login User user) {
	...
	@PutMapping(value = "/{id}")
	public ResponseEntity<Void> changeTodo(@PathVariable int id, @RequestBody TodoDto.Request todoDto,
		@Login User user) {
